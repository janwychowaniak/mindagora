...
| `LogoutButton.tsx` | Przycisk „Logout" (`variant="ghost"`), `fetch("/api/auth/logout", { method: "POST" })`, potem `window.location.assign("/login")`. `401` (sesja już nieważna) liczy się jak sukces — cel kliknięcia osiągnięty, więc też przekierowanie; `api-client` celowo nie przekierowuje sam dla `/api/auth/*`. Inny błąd → komunikat inline, sesja pozostaje.                   |
